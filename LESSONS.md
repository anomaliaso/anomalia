...
**Vale identico per i test, e lì si traveste da difetto vero.** Basta che il checkout madre sia indietro di qualche commit, e un test che confronta due lati — uno dal worktree, uno dal pacchetto — accusa un lato sano. Due incontrati nello stesso pomeriggio: `job-roster.test.ts` con `TypeError: AUTOMATION_JOBS is not iterable` (la costante c'è su `dev`, non nella copia che il symlink risolve), e `shared-views.test.ts` che confronta il vincolo `CHECK` di una migration del worktree (5 tipi) con `SHARED_VIEW_TYPES` del contratto madre (3) e sembra drift fra codice e schema. **Non lo è**: in CI, che fa un checkout di UN commit, i due lati sono lo stesso.

Segnale: un rosso su un file che non hai toccato, dove il valore «sbagliato» è quello VECCHIO e il tuo è nuovo. Mossa, in ordine di costo: `ls -la node_modules/@anomalia/` per vedere dove punta davvero, poi rieseguire lo stesso test in un worktree che NON contiene la tua modifica — se fallisce identico non è tuo. Dichiararlo senza quella seconda esecuzione è la parte che va evitata: «rosso preesistente» detto e non verificato è come un test mai lanciato.
