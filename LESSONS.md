...
### Due agenti nella STESSA directory di lavoro: il difetto dello stash senza lo stash
Il `git stash` è vietato perché due processi condividono un ref. La stessa cosa succede, senza stash, quando due agenti lavorano nella **stessa working directory** — e lì non c'è nessun hook che lo fermi. È successo il 5/9: un agente stava modificando la sua branch nel suo worktree, un secondo agente ha fatto merge e poi ha avviato un `git rebase` **dentro quella stessa directory**. L'edit non committato del primo è sparito a metà frase.

Segnale: il file che stavi scrivendo cambia sotto di te, `git status` mostra `UU` su file che non hai toccato, e `.git/rebase-merge/head-name` nomina la TUA branch mentre `HEAD` è il commit di qualcun altro. Segnale precoce: un peer che ti dice «ribaso sulla tua branch» senza dirti *dove*.

Mossa, in quest'ordine: **non abortire** il rebase altrui — `git rebase --abort` in una directory condivisa distrugge il lavoro in volo di chi ce l'ha messo, che è il danno peggiore del danno che stai riparando. Lascia finire, poi chiedi che si sposti nel proprio worktree. Una branch condivisa va bene: `git fetch` e `git rebase` dalla PROPRIA directory sono operazioni sicure e non si vedono fra loro. È la directory a non essere condivisibile, non il branch.

Prevenzione: `git worktree add` costa due secondi e un worktree per agente è la regola già scritta in CLAUDE.md — questa riga esiste perché quella regola parlava di branch e qualcuno ha letto «stessa branch, stessa cartella» come consentito.
