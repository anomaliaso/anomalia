...
### Un modello che ragiona sta zitto più di quanto il socket sia disposto ad aspettare
Il `fetch` di Node chiude dopo 300 secondi di silenzio (`bodyTimeout`/`headersTimeout` di undici), e un modello che ragiona manda `200 OK` e poi niente per minuti. L'errore che ne esce — `Failed to process successful response`, `terminated`, `ETIMEDOUT`, a volte `ECONNRESET` — sembra un guasto del provider, ed è costato due diagnosi sbagliate: prima «il modello è rotto», poi «l'ho rotto io con la modifica in PR». Il timeout dell'AI SDK (`abortSignal`) non protegge: governa un altro strato, e quello sotto chiude comunque. Segnale: il fallimento arriva quasi esattamente a 300s (~5 minuti) e solo sui percorsi lunghi (cicli d'agente, schemi grandi), mentre le chiamate brevi non sbagliano mai. Mossa: dare al client un `fetch` con un `Agent` undici i cui `bodyTimeout`/`headersTimeout` seguono il timeout del prodotto — e passarlo attraverso `globalThis.fetch`, non importando `fetch` da `undici`, altrimenti i test che sostituiscono il fetch globale non intercettano più niente.

### Un test che scrive a mano una «data futura» marcisce col calendario
`scheduled_for: '2026-09-01T10:00'` passava alle 09:12 e falliva alle 10:30 dello stesso giorno: la data era diventata passato durante la sessione. Il rosso sembrava una regressione della modifica in corso, che non c'entrava. Segnale: un test verde a inizio sessione e rosso a fine, su un file che nessuno ha toccato. Mossa: la data si calcola da `Date.now()`, come l'ordine atteso si calcola invece di scriverlo a memoria — stessa regola, altro tipo di dato.

### Lo stesso nome di livello vuol dire cose opposte in due sistemi
`thinkingLevel: 'high'` su Gemini è un TETTO — nasce per fermare un giudice che «lasciato libero ragionava 9x l'output». Sul gateway OpenRouter, `reasoning: {effort: 'high'}` è il MASSIMO. Mappare il primo sul secondo sembra rispettare il punto di chiamata e invece ne ribalta l'intenzione: la revisione di tre caption è passata da un verdetto secco a 420 secondi, poi a un timeout di 900. Segnale: un parametro che porta lo stesso nome in due strati tarati su modelli diversi. Mossa: non tradurre una manopola da un sistema all'altro senza misurare cosa vuol dire il valore di là; e diffidare del default di una costante (`DEFAULT_GEMINI_THINKING = 'high'`) quando viaggia fuori dal sistema per cui è stata scelta.
