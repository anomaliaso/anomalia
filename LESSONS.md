...
### La data «futura» scritta a mano nel test scade, e il rosso sembra tuo
`scheduled_for: '2026-09-01T10:00'` era nel futuro il giorno in cui il test è stato scritto; il 1° settembre 2026 alle 11 è nel passato, e `web_schedule_article` + `content_reschedule_post` falliscono con `requested time is in the past` su un albero che non li tocca nemmeno di striscio. Segnale: fallimento deterministico, in file estranei al diff, con un messaggio che parla di tempo. Mossa: prima di indagare, `grep -n "20[0-9][0-9]-" <test>` — una data letterale è una bomba a orologeria; l'unica che non scade è relativa (`new Date(Date.now() + ...)`).
