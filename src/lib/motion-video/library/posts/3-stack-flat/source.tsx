// wow: SLIDE_INERTIA — pila verticale PIATTA: le card scorrono una sotto l'altra, senza prospettiva.
//
// È LA LINEA DI BASE, e serve proprio per quello. Nessuna rotazione, nessuna profondità: una
// colonna di card che scorre. Quando il contenuto è già forte — foto vere, un carosello che
// qualcuno ha approvato — la geometria in più toglie invece di aggiungere, e questa è la voce
// da prendere. Le altre quattro di `posts/` sono la stessa pila con una trasformazione sopra: se
// non sai quale scegliere, parti da qui e passa a un'altra solo se hai una ragione.
//
...
//
// IL RAGGIO È IN PIXEL, FISSO: 24 su una card, dentro la banda 16-24 della specifica. Mai una
...
const BEAT = 2.4;
const STEPS = 2;
const RUN = 1.3 * fps;   // fine della corsa: da qui in poi comanda la molla
...
const Stage: React.FC<{ from: number }> = ({ from }) => {
	...
	const drift = interpolate(frame, [0, BEAT * fps], [-14, 14], { easing: EXPO, ...CLAMP });
		...
		<AbsoluteFill style={{ backgroundColor: GROUND, justifyContent: 'center', alignItems: 'center', overflow: 'hidden' }}>
			...
			>
				{SLIDES.map((src, i) => {
					...
					// PIATTA: solo scorrimento verticale e un filo di rimpicciolimento sulle vicine.
					// Nessuna prospettiva — è l'assenza di geometria a essere la scelta.
					const place =
						'translateY(' + (offset * (CARD_H + 44) + drift) + 'px) scale(' + (1 - Math.min(Math.abs(offset), 2) * 0.12) + ')';
					const op = 1 - Math.min(Math.abs(offset), 2) * 0.45;
					return (
						<div
							key={src}
							style={{
...
export default function StackFlatDemo() {
	const { fps } = useVideoConfig();
