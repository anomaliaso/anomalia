// wow: FULL_CANVAS_SCALE — corridoio: pila verticale in prospettiva, le card sopra e sotto trapezoidali che rientrano in profondità.
//
// LA PILA CHE DIVENTA UN CORRIDOIO. Stessa colonna di `3-stack-flat`, ma con la prospettiva
// accesa e una rotazione su X piccola: le card sopra e sotto non si ribaltano — RIENTRANO, e la
// prospettiva le disegna come trapezi. Chi guarda non vede una lista che scorre, vede uno spazio
// in cui si avanza.
//
// PERCHÉ' NON È IL ROLODEX. Lì la rotazione supera i novanta gradi e la card mostra il retro:
// è un oggetto che gira. Qui la rotazione resta sotto i trenta e la faccia è sempre leggibile:
// è un oggetto che si allontana. Sono due letture diverse dello stesso asse, e la differenza sta
// tutta in quanti gradi si ruota e quanto si spinge in profondità.
//
...
//
// IL RAGGIO È IN PIXEL, FISSO: 22 su una card, dentro la banda 16-24 della specifica. Mai una
...
const Stage: React.FC<{ from: number }> = ({ from }) => {
	...
	const drift = interpolate(frame, [0, BEAT * fps], [-12, 12], { easing: EXPO, ...CLAMP });
		...
		<AbsoluteFill style={{ backgroundColor: GROUND, justifyContent: 'center', alignItems: 'center', overflow: 'hidden' }}>
			...
			>
				{SLIDES.map((src, i) => {
					...
					// PROSPETTIVA, NON RIBALTAMENTO: rotazione piccola e translateZ grande. La faccia
					// resta leggibile e la prospettiva la disegna come un trapezio che rientra.
					const place =
						'translateY(' + (offset * 940 + drift) + 'px) translateZ(' + (-Math.abs(offset) * 1000) + 'px) rotateX(' + (offset * 26) + 'deg)';
					const op = 1 - Math.min(Math.abs(offset), 3) * 0.26;
					return (
						<div
							key={src}
							style={{
...
export default function CorridorDemo() {
	const { fps } = useVideoConfig();
