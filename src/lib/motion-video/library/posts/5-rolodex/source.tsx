// wow: PUSH_ZOOM_PARALLAX — rolodex verticale: la centrale frontale, quella sopra e quella sotto ribaltate su X e viste di scorcio.
//
// È IL COVERFLOW RUOTATO DI NOVANTA GRADI, e cambia di senso. La slide sopra e quella sotto si
// ribaltano su X fino a mostrare il rovescio, viste quasi di taglio: la pila diventa uno schedario
// che gira. Rispetto al coverflow orizzontale legge come TEMPO — prima, adesso, dopo — invece che
// come catalogo, ed è per questo che vale una voce a se'.
//
// IL RETRO SI DEVE VEDERE. Sopra i novanta gradi la card mostra il proprio rovescio specchiato:
// è la parte dell'effetto che fa capire che gli oggetti sono fisici. `backfaceVisibility` resta
// `visible`.
//
		...
		<AbsoluteFill style={{ backgroundColor: GROUND, justifyContent: 'center', alignItems: 'center', overflow: 'hidden' }}>
			...
			>
				{SLIDES.map((src, i) => {
					...
					// L'ASSE VERTICALE: y cresce con la distanza, z rientra, e la rotazione su X porta
					// le vicine di scorcio e poi oltre il taglio, dove si vede il rovescio.
					const turn = Math.max(-150, Math.min(150, offset * 72));
					const place =
						'translateY(' + (offset * 700 + drift) + 'px) translateZ(' + (-Math.abs(offset) * 360) + 'px) rotateX(' + turn + 'deg)';
					const op = 1 - Math.min(Math.abs(offset), 3) * 0.2;
					return (
						<div
							key={src}
							style={{
...
export default function RolodexDemo() {
	const { fps } = useVideoConfig();
