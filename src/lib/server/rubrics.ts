...
- CAST — a narrative series names its RECURRING protagonist inside art_direction, drawn concretely enough to be redrawn identically every episode: silhouette, hair, habitual clothes, one object that is always with them. Describe a character, never a demographic: no ethnicity, no body type, no age bracket. A series whose protagonist changes face between one episode and the next is not a series, and it is the single most common way a drawn series stops reading as one. Fix only what never changes: if an episode is ABOUT a change to the character (a haircut, a scar, a new coat), the story wins over the description, so do not pin the very trait the stories are likely to turn on.
