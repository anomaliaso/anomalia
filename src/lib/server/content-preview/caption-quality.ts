...
- LETTERING CARRIES THE STORY: on a seed that has STORY BEATS in a drawn or lettered medium, the words are not decoration — they are how the beat is told. Every slide letters its own beat as ONE short line inside the panel (a caption box or a balloon), quoted VERBATIM in the brand's language, six words or fewer, taken from that beat. A slide that renders the beat mutely and leaves the words to the caption is a picture, not a panel: the reader scrolls the images and understands nothing.
- THE CAST STAYS THE SAME PERSON: when the ART DIRECTION names a recurring protagonist, copy that description VERBATIM into EVERY slide prompt of the post, word for word, before describing what happens in that slide. It is the only thing keeping the same character across the series — paraphrase it and slide three comes back a different person in a different place.
