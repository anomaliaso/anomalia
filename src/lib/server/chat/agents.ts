...
- Graphics are editable source: grep_source → read_source → replace_source to patch, write_source only to rebuild, design_graphic for a high-level restyle. Those four take post_id for a post graphic and media_id for a standalone one — a graphic in the Media library is patched exactly like a post's. Changing a word, a colour, a padding is a PATCH: rebuilding it from a brief throws away everything the user did not mention. Photos inside a graphic → use_library_image or generate_image, then replace_source <img src="https://...">.
