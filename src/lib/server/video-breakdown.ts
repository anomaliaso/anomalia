...
/** Text-only fallback when multimodal breakdown is unavailable — still useful for agent tooling. */
export async function breakdownReferenceVideoTextOnly(
  description: string
  ...
  try {
    ...
    const parsed = await structured(
      ...
      BREAKDOWN_SCHEMA,
      undefined,
      { label: 'ugc.breakdown.text' }
    );
    const beatsRaw = Array.isArray((parsed as { beats?: unknown }).beats)
      ? ((parsed as { beats: Array<Record<string, unknown>> }).beats)
      : [];
    const brief: UgcShotBrief = {
      subject: String((parsed as { subject?: unknown }).subject ?? '').trim(),
      camera: String((parsed as { camera?: unknown }).camera ?? '').trim(),
      audio: String((parsed as { audio?: unknown }).audio ?? '').trim(),
      behavioralBeats: pickUgcBehavioralBeats(
        `${(parsed as { subject?: unknown }).subject ?? ''}|${(parsed as { dialogue_summary?: unknown }).dialogue_summary ?? ''}`
      ),
      timeline: beatsRaw
        .map((b) => ({
          start: Number(b.start) || 0,
    ...
    if (!brief.subject) return null;
    return {
      brief,
      prompt: formatUgcShotBrief(brief),
      dialogueSummary: String((parsed as { dialogue_summary?: unknown }).dialogue_summary ?? '').trim(),
      durationSeconds: Number((parsed as { duration_seconds?: unknown }).duration_seconds) || 15
    };
  } catch {
    return null;
  }
