...
export function createContentTools(ctx: ChatToolCtx) {
  ...
  return {
    ...
    motion_control_video: tool({
      description:
        'Take the MOVEMENT from one video and apply it to the subject of an image: your product, person or character performs what the reference clip does. Two different inputs and they are not interchangeable — image_url is WHO moves, video_url is HOW they move. Use it to reproduce a performance, a camera move or a dance on the brand\'s own subject. The reference video must contain a PERSON — kie refuses a clip with no human in it ("No valid characters detected in the video") after a full round trip, so a product shot or an abstract animation is not a motion reference. This is NOT a motion video: those are Remotion compositions rendered from TSX code (motion_write) and use no generative model at all. Runs a real render and bills credits. A real person as the subject needs recorded consent on their card in Studio → People.',
      inputSchema: z.object({
