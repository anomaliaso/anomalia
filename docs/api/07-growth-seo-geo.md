...
## `POST /api/v1/brands/:slug/videos/review`

Recensisce un video (URL pubblico o media di un post) contro lo standard `organic` o `ads`, con giudice multimodale Gemini. **Consuma crediti.** Il risultato viene persistito.

**Body**

| Campo | Tipo | Obbligatorio | Descrizione |
|---|---|---|---|
| `url` | string | No* | URL pubblico del video (https) |
| `post_id` | string | No* | UUID post: usa `media_url`, `caption`, `product_name` |
| `standard` | string | No | `organic` (default) \| `ads`; alias accettati: `ugc`, `reel`, `organic_ugc` → organic; `ad`, `ugc_ad`, `paid`, `paid_ads` → ads |
| `ugc_ad` / `ugcAd` | boolean | No | Se `true` forza standard `ads` |
| `product` | string | No | Prodotto/offerta (sovrascrive quello del post) |
| `caption` | string | No | Caption (sovrascrive) |
| `script` | string | No | Script parlato previsto |

\* obbligatorio almeno uno tra `url` e `post_id` (o un post con media).

**Response** `200` — `review` è l'output del giudice AI (dinamico):
...
```json
{
  ...
  "review": {
    ...
    "overall": 6.4,
    "duration_s": 21.0,
    "doomscroll": { "stops": true, "who": "<dinamico>", "reason": "<dinamico>" },
    "hook": { "at_s": 1.2, "type": "pain_callout", "line": "<dinamico>", "visual": "<dinamico>", "callout": true, "open_loop": true, "promise_match": true, "unique": false },
    "reveal_at_s": 9.5,
    "cta_at_s": 19.0,
    "dead_seconds": [4, 5],
    "scores": { "scroll_stop": 7, "sound_off": 6, "hold": 6, "authenticity": 8, "structure": 5, "spoken_craft": 6, "cta_soft": 4, "loop_worthiness": 5 },
    "weakest_link": "cta_soft",
    "issues": [ { "dimension": "cta_soft", "severity": "major", "at_s": 19.0, "problem": "<dinamico>", "fix": "<dinamico>" } ],
    "next_test": "<dinamico>",
    "summary": "<dinamico>",
    "script": { "spoken": "<dinamico>", "on_screen": "<dinamico>", "caption": "<dinamico>" },
    "judgment": "<dinamico>"
  }
}
```

Note: `verdict` ∈ `ship`|`fix`|`kill`. Per standard `ads`, `scores` usa `audience_signal, proof, offer, uniqueness, claims_safe` al posto di `cta_soft, loop_worthiness`.
...
| `422` | `{"error":"<errore>"}` (es. `gemini_unconfigured`, `media_extract_failed`, `model_failed`, `model_parse_failed`) |

**Esempio**:

```bash
curl -s -X POST "https://anomalia.so/api/v1/brands/mio-brand/videos/review" \
  -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
  -d '{"url":"https://cdn.example.com/clip.mp4","standard":"ads","product":"CRM per PMI"}'
```
